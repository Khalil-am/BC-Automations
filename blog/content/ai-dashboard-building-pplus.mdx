...
The **pplus-knowledge-mcp** is a Model Context Protocol server that gives Claude Code 8 specialized tools for querying the PPlus v4 knowledge base. Instead of Claude reading raw markdown files, it can search, filter, cross-reference, and retrieve exactly the information it needs — instantly.

### MCP Tools Available

| Tool | What It Does |
|---|---|
| `search_knowledge` | Natural language search across all 36 knowledge chunks — architecture, APIs, frontend, backend, admin, data models, instances, integrations |
...
| `get_instance_config` | Get instance-specific configuration (Masar, Cruise Saudi, C2/Cluster2, G-PMO, Diwan) — levels, dashboards, branding, logs |
...
```bash
cd ~/OneDrive/Desktop/pplus-knowledge-mcp
...
Create or edit the file `~/.claude/.mcp.json`:
...
```json
{
  "mcpServers": {
    "pplus-knowledge": {
      "command": "node",
      "args": ["C:\\Users\\YourName\\pplus-knowledge-mcp\\dist\\index.js"],
      "env": {
        "PPLUS_KNOWLEDGE_PATH": "C:\\Users\\YourName\\pplus-agent-corag-vectorized\\knowledge"
      }
    }
  }
}
...
Close and reopen Claude Code. You should see the `pplus-knowledge` MCP server connect on startup. Claude now has permanent access to all 36 knowledge chunks across 8 categories.
...
The [pplus-agent-corag-vectorized](https://github.com/Khalil-am/pplus-agent-corag-vectorized) repository is structured as a RAG (Retrieval-Augmented Generation) knowledge base for AI agents. It contains **36 knowledge chunks across 8 categories**:
...
knowledge/
  ...
  frontend/            (8 chunks) Dashboard builder, entity info, Gantt, tasks center
  backend/             (2 chunks) ChartBuilder pipeline, authentication
  admin-panel/         (9 chunks) All 12 admin tabs, 50+ card toggles, process builder
  data-models/         (2 chunks) Level hierarchy, dynamic property system
  integrations/        (1 chunk)  Module federation, micro-frontend setup
  instances/           (5 chunks) Masar, Cruise Saudi, C2/Cluster2, G-PMO, Diwan configs
```
...
| Frontend | 8 | Dashboard builder, 17 field types, Gantt, tasks center, executive widgets |
| Backend | 2 | 7-stage ChartBuilder pipeline, token auth flow |
| Admin Panel | 9 | 12 admin tabs, 50+ toggles, process builder, 52 lookups, RBAC tree |
...
| Instances | 5 | Full configs for 5 client deployments with level hierarchies, branding, log modules |
...
3. **Contribute back:** Push new property mappings and instance discoveries to the [knowledge repository](https://github.com/Khalil-am/pplus-agent-corag-vectorized) so the entire team benefits
