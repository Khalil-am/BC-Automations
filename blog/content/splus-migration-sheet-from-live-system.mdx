...
description: "Automatically generate a complete, form-accurate data migration Excel workbook by inspecting any live P+ instance — extracting the exact hierarchy, creation forms, and all sub-item (log) field definitions so clients can fill and import data without guesswork."
date: "2026-04-05"
tags: ["PPlus", "Migration", "Data Import"]
featured: true
...
2. **Reference template** (optional) — e.g., an existing migration `.xlsx` for style matching
...
Some forms show/hide fields based on dropdown selections (e.g., KPI Type changes available fields). The automation captures the default state of the form. If you need conditional fields included, specify in the prompt:
...
Once the migration sheet is filled out by the client, it can be used with the **P+ Data Import** automation to bulk-create all entities in the target system via API. The column headers in the migration sheet are designed to map directly to the system's creation form fields, ensuring a smooth handoff from data collection to data import.
